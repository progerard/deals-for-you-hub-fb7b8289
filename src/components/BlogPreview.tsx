            ...
            <motion.article key={post.slug}
              ...
              <Link to={`/blog/${post.slug}`} className="block card-product group p-6 h-full hover:border-primary/40">
                <div className="flex flex-col h-full">
                  ...
                  <h3 className="font-display text-lg font-bold text-foreground mb-3 group-hover:text-primary transition-colors leading-tight">{post.title}</h3>
                  <p className="text-muted-foreground text-sm leading-relaxed mb-4 flex-1 line-clamp-3">{post.description}</p>
                  <div className="flex items-center justify-between text-xs text-muted-foreground pt-4 border-t border-border/50">
                    <div className="flex items-center gap-3">
                      <span className="flex items-center gap-1">
                        <Calendar className="h-3 w-3" />
                        {new Date(post.date).toLocaleDateString(locale, { day: "numeric", month: "short" })}
                      </span>
                      <span className="flex items-center gap-1">
