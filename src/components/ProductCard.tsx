...
const ProductCard = ({ name, price, features, popular, lifetime, annual, duration, index = 0 }: ProductCardProps) => {
  ...
  return (
    ...
    >
      {popular && (
        <motion.div className="absolute -top-px left-1/2 -translate-x-1/2 z-20"
          initial={{ opacity: 0, y: -20, scale: 0.8 }} whileInView={{ opacity: 1, y: 0, scale: 1 }}
          viewport={{ once: true }} transition={{ delay: index * 0.12 + 0.3, type: "spring", stiffness: 200 }}>
          <div className="bg-gradient-primary px-6 py-2 rounded-b-xl shadow-glow">
            <span className="text-xs font-bold text-primary-foreground flex items-center gap-1.5 uppercase tracking-wider">
              <Crown className="h-3.5 w-3.5" /> {t("card.popular")}
            </span>
          </div>
        </motion.div>
      )}
        ...
        <motion.div className="absolute -top-px left-1/2 -translate-x-1/2 z-20"
          ...
          <div className="bg-gradient-accent px-6 py-2 rounded-b-xl shadow-glow-accent">
            <span className="text-xs font-bold text-white flex items-center gap-1.5 uppercase tracking-wider">
              <Sparkles className="h-3.5 w-3.5" /> {t("card.lifetime")}
            </span>
          </div>
        </motion.div>
      )}
      ...
      <div className="relative z-10 flex flex-col h-full">
        <div className="mb-8 pt-4 text-center">
          <motion.h3 className="font-display text-2xl font-bold text-foreground mb-4"
            initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }}
            transition={{ delay: index * 0.12 + 0.2 }}>
            {name}
          </motion.h3>
          <motion.div className="flex items-baseline gap-2 justify-center"
            initial={{ opacity: 0, scale: 0.8 }} whileInView={{ opacity: 1, scale: 1 }}
            viewport={{ once: true }} transition={{ delay: index * 0.12 + 0.25, type: "spring", stiffness: 150 }}>
            <span className="text-5xl font-extrabold text-gradient">{price}€</span>
            {annual && <span className="text-base text-muted-foreground font-medium">{t("card.perYear")}</span>}
        ...
        <ul className="space-y-4 inline-block text-left flex-1">
          {features.map((feature, featureIndex) => (
            <motion.li key={featureIndex} className="flex items-start gap-4"
              initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}
              transition={{ duration: 0.4, delay: (index * 0.12) + (featureIndex * 0.08) + 0.35 }}>
              <motion.div className="flex-shrink-0 w-6 h-6 rounded-full bg-primary/20 flex items-center justify-center mt-0.5 group-hover:bg-primary/30 transition-colors duration-300"
        ...
        <div className="mt-8 pt-6 border-t border-border/50 flex gap-3">
          <motion.a
            href={`https://wa.me/34640329880?text=${encodeURIComponent(t("wa.productMsg", { name, price }))}`}
            target="_blank" rel="noopener noreferrer"
            className={`flex-1 py-3.5 rounded-xl font-bold text-base flex items-center justify-center gap-2 transition-all duration-300 ${popular ? 'bg-[#25D366] text-white hover:bg-[#1fb855]' : 'bg-[#25D366]/15 text-[#25D366] hover:bg-[#25D366]/25'}`}
          ...
          <motion.a
            ...
            className={`flex-1 py-3.5 rounded-xl font-bold text-base flex items-center justify-center gap-2 transition-all duration-300 ${popular ? 'bg-[#2AABEE] text-white hover:bg-[#229ED9]' : 'bg-[#2AABEE]/15 text-[#2AABEE] hover:bg-[#2AABEE]/25'}`}
            whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}>
